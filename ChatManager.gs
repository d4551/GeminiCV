 ...
 */
function applyTemplateToDocument(documentId, templateId) {
  try {
    ...
    return { error: true, message: `Could not apply template: ${error.toString()}` };
  }
}

/**
 * Apply a specific template design to the document
 */
function applyGoogleXYZFormat(documentId) {
  return applyTemplateToDocument(documentId, "google-xyz");
}
...
function applyTemplateToCurrentDocument(templateId) {
  ...
  return applyTemplateToDocument(docId, templateId);
}
...
/**
 * Get all available templates including user's custom styles
 */
function getAllTemplates() {
  try {
    ...
    // Add user's custom styles if available
    const props = PropertiesService.getUserProperties();
    const userSettings = JSON.parse(props.getProperty(USER_PROPS_KEYS.SETTINGS) || '{}');
    
    if (userSettings.customStyles) {
      Object.assign(templates, userSettings.customStyles);
    }
...
class StyleManager {
        ...
        if (level === DocumentApp.ParagraphHeading.HEADING1) {
          ...
          if (templateName === 'modern-professional' || templateName === 'minimalist') {
            ...
            try {
              ...
              console.log('Border setting not supported, using fallback');
            }
          }
  ...
  static getAvailableTemplates() {
    try {
      ...
      // Add all built-in templates from RESUME_TEMPLATES
      for (const [id, template] of Object.entries(RESUME_TEMPLATES)) {
        templates[id] = {
          name: template.name,
          description: template.description,
          // Generate preview URL or use placeholder
          preview: template.preview || `https://example.com/previews/${id}.png`
        };
      }
      ...
      for (const paragraph of paragraphs) {
        ...
        if (indentLevel > 0) {
          ...
          try {
            ...
            console.log('Border setting not supported for quotes, using fallback');
          }
        }
