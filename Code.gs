...
/**
 * Shows the sidebar with improved document awareness
 * @return {void}
 */
...
function showAbout() {
  const html = HtmlService.createHtmlOutput(`
    <div style="padding: 20px; font-family: Arial, sans-serif;">
      ...
      <p>Version 1.0</p>
      <p>A Google Docs add-on for creating professional resumes and cover letters with Gemini AI.</p>
 ...
 */
async function improveDocumentWithAI(prompt) {
  // Check if API key exists
  const apiKey = PropertiesService.getUserProperties().getProperty(USER_PROPS_KEYS.API_KEY);
...
function validateAndInitializeModels(apiKey) {
  try {
    ...
    if (responseCode !== 200) {
      return { 
        valid: false, 
        message: `API Error: ${responseCode} - ${response.getContentText()}` 
      };
    }
    
    const jsonResponse = JSON.parse(response.getContentText());
    const geminiModels = jsonResponse.models
