...
/**
 * GeminiApp class for interacting with Gemini AI API
 */
class GeminiApp {
  constructor(apiKey) {
    this.apiKey = apiKey;
...
/**
 * Generate text using Gemini API with improved error handling
...
function generateWithGemini(prompt, options = {}) {
  ...
  try {
    ...
    if (!result.error) {
      console.log(`Generated content in ${executionTime}ms using model: ${modelName}`);
    }
...
/**
 * Generate content with user preferences from settings with enhanced error handling
 * @param {string} prompt - Text prompt
...
/**
 * Create a Gemini chat instance with user preferences and enhanced error handling
 * @param {Array} history - Initial chat history
...
/**
 * Format raw model data for UI display with enhanced grouping and information
...
/**
 * Validate API key and initialize available models with improved error handling
...
function generateResumeContent(userInfo) {
  const systemPrompt = `You are a professional resume writer. Create detailed, professional content for each section of a resume based on the information provided. Focus on achievements and quantifiable results where possible.`;
  ...
  1. Summary/Profile
  2. Experience (with bullet points)
  3. Education
  4. Skills
...
function generateTargetedDocuments(resumeContent, jobPostingUrl) {
  const systemPrompt = `You are an expert in resume tailoring and cover letter writing. Your goal is to help the user create highly targeted documents that align with a specific job posting, highlighting relevant skills and experiences.`;
  ...
  3. Identify 5-10 keywords from the job posting that should be included in both documents.
 ...
 * This is a centralized function that should be called when validating the API key
 */
function validateAndInitializeModels(apiKey) {
  try {
    const url = `https://generativelanguage.googleapis.com/v1beta/models?key=${apiKey}`;
    ...
    console.error(`Exception validating API key: ${error}`);
    return { valid: false, message: `Exception: ${error.toString()}` };
  }
}
 ...
 * This function ensures all user settings are applied
 */
function generateWithUserSettings(prompt, customOptions = {}) {
  const props = PropertiesService.getUserProperties();
  const settings = JSON.parse(props.getProperty(USER_PROPS_KEYS.SETTINGS) || '{}');
