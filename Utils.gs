  ...
  try {
    ...
    if (!companyName || !position) {
      try {
        ...
        console.log("AI extraction failed, using basic extraction results");
      }
    }
...
Return the complete enhanced version while maintaining the original message and facts.`;
...
function getCachedOrCompute(key, computeFunc, options = {}) {
  ...
  try {
    ...
    try {
      ...
      console.log("Error caching value:", cacheError);
    }
...
${content.substring(0, 5000)}`;
  ...
  try {
    ...
    throw new Error(`Error parsing AI extraction result: ${formatError(parseError)}`);
  }
}